# kindergarten_manager_persian_reports.py

# ====================================================================================
# توضیحات و پیش‌نیازها
# ====================================================================================
# نسخه پیشرفته با تاریخ شمسی و گزارشات کامل
...
# ====================================================================================
# کلاس ابزار تاریخ شمسی
# ====================================================================================
def to_jalali_str(gregorian_date_str):
    """رشته تاریخ میلادی YYYY-MM-DD را به رشته شمسی YYYY/MM/DD تبدیل می‌کند."""
    if not gregorian_date_str:
        return ""
    try:
        greg_date = datetime.strptime(gregorian_date_str, '%Y-%m-%d').date()
        jalali_date = jdatetime.date.fromgregorian(date=greg_date)
        return jalali_date.strftime('%Y/%m/%d')
    except (ValueError, TypeError):
        return gregorian_date_str # اگر فرمت اشتباه بود، خود رشته را برگردان

class JalaliDateEntry(ttk.Frame):
    """ویجت سفارشی برای انتخاب تاریخ شمسی."""
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        
        self.j_today = jdatetime.date.today()
        ...
        # سال
        self.year_combo = ttk.Combobox(self, width=6, values=[str(y) for y in range(self.j_today.year - 20, self.j_today.year + 5)], textvariable=self.year_var, state='readonly')
        self.year_combo.pack(side='right', padx=2)
        
        # ماه
        self.month_combo = ttk.Combobox(self, width=4, values=[str(m) for m in range(1, 13)], textvariable=self.month_var, state='readonly')
        self.month_combo.pack(side='right', padx=2)
        
        # روز
        self.day_combo = ttk.Combobox(self, width=4, values=[str(d) for d in range(1, 32)], textvariable=self.day_var, state='readonly')
        self.day_combo.pack(side='right', padx=2)
        
        self.month_combo.bind("<<ComboboxSelected>>", self._update_days)
        self.year_combo.bind("<<ComboboxSelected>>", self._update_days)

        self.set_date(self.j_today)
    ...
    def get(self):
        """یک آبجکت jdatetime.date را برمی‌گرداند."""
        try:
            return jdatetime.date(int(self.year_var.get()), int(self.month_var.get()), int(self.day_var.get()))
        except (ValueError, TypeError):
            return None

    def get_gregorian_str(self):
        """رشته تاریخ میلادی در فرمت YYYY-MM-DD را برای ذخیره در دیتابیس برمی‌گرداند."""
        j_date = self.get()
        if j_date:
    ...
    def set_date(self, j_date):
        """تاریخ ویجت را بر اساس یک آبجکت jdatetime.date تنظیم می‌کند."""
        if isinstance(j_date, jdatetime.date):
            self.year_var.set(str(j_date.year))
            self.month_var.set(str(j_date.month))
            self.day_var.set(str(j_date.day))
            self._update_days()
        elif isinstance(j_date, str) and j_date:
             try:
                greg_date = datetime.strptime(j_date, '%Y-%m-%d').date()
                j_date_obj = jdatetime.date.fromgregorian(date=greg_date)
                self.set_date(j_date_obj)
             except (ValueError, TypeError):
...
class KindergartenApp(ttk.Frame):
    # ... (بقیه ثابت‌ها و استایل‌ها بدون تغییر باقی می‌مانند)
    ...
    def setup_styles(self):
        ...
        self.style.configure('TCombobox', font=self.BODY_FONT, padding=5, relief='flat') # Padding adjusted for JalaliDateEntry
    ...
    def show_frame(self, page_name):
        ...
        elif page_name == "گزارشات": self.generate_financial_summary_report() # بارگذاری گزارش پیش‌فرض
    ...
    # ====================================================================================
    # بخش گزارشات (بازطراحی شده)
    # ====================================================================================
    def create_report_tab(self):
        report_tab = ttk.Frame(self.content_frame, style='TFrame', padding=15)
        
        # نوت‌بوک برای دسته‌بندی گزارشات
        report_notebook = ttk.Notebook(report_tab)
        report_notebook.pack(expand=True, fill='both')

        # ایجاد هر تب گزارش
        fin_summary_frame = self._create_financial_summary_frame(report_notebook)
        income_report_frame = self._create_income_report_frame(report_notebook)
        expense_report_frame = self._create_expense_report_frame(report_notebook)
        student_report_frame = self._create_student_report_frame(report_notebook)
        staff_report_frame = self._create_staff_report_frame(report_notebook)

        report_notebook.add(fin_summary_frame, text="خلاصه مالی")
        report_notebook.add(income_report_frame, text="گزارش درآمدها")
        report_notebook.add(expense_report_frame, text="گزارش هزینه‌ها")
        report_notebook.add(student_report_frame, text="گزارش دانش‌آموزان")
        report_notebook.add(staff_report_frame, text="گزارش پرسنل")
        
        return report_tab

    def _create_financial_summary_frame(self, parent_notebook):
        report_tab = ttk.Frame(parent_notebook, style='TFrame', padding=25)
        report_tab.rowconfigure(1, weight=1); report_tab.columnconfigure((0, 1), weight=1)
        control_frame = ttk.Frame(report_tab)
        control_frame.grid(row=0, column=0, columnspan=2, sticky='ew', pady=(0, 15))
        ttk.Button(control_frame, text="به‌روزرسانی گزارش", command=self.generate_financial_summary_report).pack(side='right', padx=5)
    ...
    def _create_report_frame_template(self, parent, title, load_command, export_command):
        frame = ttk.Frame(parent, padding=15)
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(1, weight=1)
        
        filter_frame = ttk.LabelFrame(frame, text="فیلترها", padding=10)
        ...
        return frame, filter_frame, tree_frame

    def _create_income_report_frame(self, parent):
        frame, filter_frame, tree_frame = self._create_report_frame_template(parent, "گزارش درآمدها", self.load_income_report, self.export_income_report)
        ttk.Label(filter_frame, text="از تاریخ:").pack(side='right', padx=(10, 2))
        self.income_rep_start_date = JalaliDateEntry(filter_frame); self.income_rep_start_date.pack(side='right', padx=2)
        ttk.Label(filter_frame, text="تا تاریخ:").pack(side='right', padx=(10, 2))
        self.income_rep_end_date = JalaliDateEntry(filter_frame); self.income_rep_end_date.pack(side='right', padx=10)
        ttk.Button(filter_frame, text="اعمال فیلتر", command=self.load_income_report).pack(side='right', padx=5)
        ttk.Button(filter_frame, text=f"{self.icons['export']} خروجی اکسل", command=self.export_income_report).pack(side='left', padx=5)
        cols = ("دانش‌آموز", "تاریخ", "مبلغ", "نوع", "توضیحات")
        headings = {"دانش‌آموز": "دانش‌آموز", "تاریخ": "تاریخ پرداخت", "مبلغ": "مبلغ (ریال)", "نوع": "نوع پرداخت", "توضیحات": "توضیحات"}
        self.income_report_tree = self._create_treeview(tree_frame, cols, headings, {}); self.income_report_tree.grid(row=0, column=0, sticky='nsew')
        return frame

    def _create_expense_report_frame(self, parent):
        frame, filter_frame, tree_frame = self._create_report_frame_template(parent, "گزارش هزینه‌ها", self.load_expense_report, self.export_expense_report)
        ttk.Label(filter_frame, text="از تاریخ:").pack(side='right', padx=(10, 2))
        self.expense_rep_start_date = JalaliDateEntry(filter_frame); self.expense_rep_start_date.pack(side='right', padx=2)
        ttk.Label(filter_frame, text="تا تاریخ:").pack(side='right', padx=(10, 2))
        self.expense_rep_end_date = JalaliDateEntry(filter_frame); self.expense_rep_end_date.pack(side='right', padx=10)
        ttk.Button(filter_frame, text="اعمال فیلتر", command=self.load_expense_report).pack(side='right', padx=5)
        ttk.Button(filter_frame, text=f"{self.icons['export']} خروجی اکسل", command=self.export_expense_report).pack(side='left', padx=5)
        cols = ("تاریخ", "شرح", "مبلغ", "دسته‌بندی")
        headings = {"تاریخ": "تاریخ", "شرح": "شرح هزینه", "مبلغ": "مبلغ (ریال)", "دسته‌بندی": "دسته‌بندی"}
        self.expense_report_tree = self._create_treeview(tree_frame, cols, headings, {}); self.expense_report_tree.grid(row=0, column=0, sticky='nsew')
        return frame

    def _create_student_report_frame(self, parent):
        frame, filter_frame, tree_frame = self._create_report_frame_template(parent, "گزارش دانش‌آموزان", self.load_student_report, self.export_student_report)
        ttk.Label(filter_frame, text="وضعیت:").pack(side='right', padx=(10, 2))
        self.student_rep_status = ttk.Combobox(filter_frame, values=["همه", "فعال", "غیرفعال"], state='readonly'); self.student_rep_status.pack(side='right', padx=10); self.student_rep_status.set("همه")
        ttk.Button(filter_frame, text="اعمال فیلتر", command=self.load_student_report).pack(side='right', padx=5)
        ttk.Button(filter_frame, text=f"{self.icons['export']} خروجی اکسل", command=self.export_student_report).pack(side='left', padx=5)
        cols = ("نام", "نام خانوادگی", "کد ملی", "تاریخ ثبت‌نام", "شماره تماس", "کلاس", "وضعیت")
        headings = {"نام": "نام", "نام خانوادگی": "نام خانوادگی", "کد ملی": "کد ملی", "تاریخ ثبت‌نام": "تاریخ ثبت‌نام", "شماره تماس": "شماره تماس", "کلاس": "کلاس", "وضعیت": "وضعیت"}
        self.student_report_tree = self._create_treeview(tree_frame, cols, headings, {}); self.student_report_tree.grid(row=0, column=0, sticky='nsew')
        return frame

    def _create_staff_report_frame(self, parent):
        frame, filter_frame, tree_frame = self._create_report_frame_template(parent, "گزارش پرسنل", self.load_staff_report, self.export_staff_report)
        ttk.Label(filter_frame, text="وضعیت:").pack(side='right', padx=(10, 2))
        self.staff_rep_status = ttk.Combobox(filter_frame, values=["همه", "فعال", "غیرفعال"], state='readonly'); self.staff_rep_status.pack(side='right', padx=10); self.staff_rep_status.set("همه")
        ttk.Button(filter_frame, text="اعمال فیلتر", command=self.load_staff_report).pack(side='right', padx=5)
        ttk.Button(filter_frame, text=f"{self.icons['export']} خروجی اکسل", command=self.export_staff_report).pack(side='left', padx=5)
        cols = ("نام", "نام خانوادگی", "کد ملی", "تاریخ استخدام", "شماره تماس", "سمت", "وضعیت")
        headings = {"نام": "نام", "نام خانوادگی": "نام خانوادگی", "کد ملی": "کد ملی", "تاریخ استخدام": "تاریخ استخدام", "شماره تماس": "شماره تماس", "سمت": "سمت", "وضعیت": "وضعیت"}
        self.staff_report_tree = self._create_treeview(tree_frame, cols, headings, {}); self.staff_report_tree.grid(row=0, column=0, sticky='nsew')
        return frame
    ...
    def generate_financial_summary_report(self):
        ...
        if any(s > 0 for s in sizes):
            self.report_ax.pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', shadow=True, startangle=90, textprops={'fontname': self.FONT_FAMILY[0], 'fontsize': 10})
        else:
            self.report_ax.text(0.5, 0.5, 'داده‌ای برای نمایش وجود ندارد', ha='center', va='center', fontdict={'fontname': self.FONT_FAMILY[0]})
        
        self.report_ax.axis('equal'); self.report_fig.tight_layout(); self.chart_canvas.draw()
        logging.info("گزارش خلاصه مالی به‌روزرسانی شد.")

    def _execute_report_query(self, tree, base_query, filters, date_cols=[], num_cols=[]):
        query = base_query
        params = []
        
        where_clauses = []
        for key, value, operator in filters:
            if value is not None and value != "همه" and value != "":
        ...
        results = self.db_manager.execute_query(query, tuple(params), fetch="all")
        if results:
            processed_results = []
            for row in results:
                new_row = list(row)
                for idx in date_cols: new_row[idx] = to_jalali_str(new_row[idx])
                for idx in num_cols: new_row[idx] = f"{new_row[idx]:,.0f}" if new_row[idx] is not None else "0"
                processed_results.append(tuple(new_row))
            self._add_items_to_treeview(tree, processed_results)
        else:
            self._add_items_to_treeview(tree, [])

    def load_income_report(self):
        query = "SELECT s.name || ' ' || s.family_name, sf.payment_date, sf.payment_amount, sf.payment_type, sf.description FROM student_finance sf JOIN students s ON sf.student_id = s.id"
        filters = [
            ('sf.payment_date', self.income_rep_start_date.get_gregorian_str(), '>='),
            ('sf.payment_date', self.income_rep_end_date.get_gregorian_str(), '<=')
        ]
        self._execute_report_query(self.income_report_tree, query, filters, date_cols=[1], num_cols=[2])

    def load_expense_report(self):
        query = "SELECT date, description, amount, category FROM kindergarten_expenses"
        filters = [
            ('date', self.expense_rep_start_date.get_gregorian_str(), '>='),
            ('date', self.expense_rep_end_date.get_gregorian_str(), '<=')
        ]
        self._execute_report_query(self.expense_report_tree, query, filters, date_cols=[0], num_cols=[2])

    def load_student_report(self):
        status_map = {"فعال": "active", "غیرفعال": "inactive"}
        status_filter = status_map.get(self.student_rep_status.get())
        query = "SELECT name, family_name, national_id, entry_date, parent_phone, class_name, status FROM students"
        filters = [('status', status_filter, '=')]
        self._execute_report_query(self.student_report_tree, query, filters, date_cols=[3])

    def load_staff_report(self):
        status_map = {"فعال": "active", "غیرفعال": "inactive"}
        status_filter = status_map.get(self.staff_rep_status.get())
        query = "SELECT name, family_name, national_id, hire_date, phone, role, status FROM staff"
        filters = [('status', status_filter, '=')]
        self._execute_report_query(self.staff_report_tree, query, filters, date_cols=[3])

    def _export_to_excel(self, tree, filename_prefix, columns):
        if not (pd and openpyxl):
            messagebox.showerror("خطا", "کتابخانه‌های 'pandas' و 'openpyxl' برای این عملیات مورد نیاز هستند.")
            return

        file_path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")], title="ذخیره گزارش اکسل", initialfile=f"{filename_prefix}_{datetime.now().strftime('%Y-%m-%d')}.xlsx")
    ...
    def export_income_report(self):
        cols = ["دانش‌آموز", "تاریخ پرداخت", "مبلغ (ریال)", "نوع پرداخت", "توضیحات"]
        self._export_to_excel(self.income_report_tree, 'income_report', cols)

    def export_expense_report(self):
        cols = ["تاریخ", "شرح هزینه", "مبلغ (ریال)", "دسته‌بندی"]
        self._export_to_excel(self.expense_report_tree, 'expense_report', cols)

    def export_student_report(self):
        cols = ["نام", "نام خانوادگی", "کد ملی", "تاریخ ثبت‌نام", "شماره تماس", "کلاس", "وضعیت"]
        self._export_to_excel(self.student_report_tree, 'student_report', cols)

    def export_staff_report(self):
        cols = ["نام", "نام خانوادگی", "کد ملی", "تاریخ استخدام", "شماره تماس", "سمت", "وضعیت"]
        self._export_to_excel(self.staff_report_tree, 'staff_report', cols)
    ...
    def _create_person_form(self, parent, labels_dict, date_keys):
        ...
        for i, (key, text) in enumerate(labels_dict.items()):
            ...
            if key == "status":
                ...
                widget = JalaliDateEntry(parent) # استفاده از ویجت جدید
            else:
    ...
    def _create_treeview(self, parent, columns, headings, widths):
        ...
        # tree.grid(row=1, column=0, sticky='nsew') # grid is now managed outside
        scrollbar = ttk.Scrollbar(parent, orient="vertical", command=tree.yview)
        tree.configure(yscrollcommand=scrollbar.set)
        scrollbar.grid(row=0, column=1, sticky='ns') # Adjusted grid for template

        for col, text in headings.items():
            tree.heading(col, text=text, anchor="center")
            tree.column(col, width=widths.get(col, 120), anchor="center") # Default width increased
        
        if "id" in columns or "ID" in headings:
             tree.column("id", stretch=tk.NO, width=40)
        return tree
    
    # ... بخش‌های دانش‌آموزان و پرسنل با تغییرات جزئی برای تاریخ شمسی ...
    ...
    def create_student_tab(self):
        ...
        self.student_tree.grid(row=1, column=0, sticky='nsew') # Place treeview
    ...
    def clear_student_form(self):
        ...
        for key, widget in self.student_entries.items():
            ...
            elif isinstance(widget, JalaliDateEntry): widget.set_date_to_today()
    ...
    def search_students(self, event=None): self.load_students(self.student_search_entry.get().strip())
    ...
    def load_student_to_form(self, event=None):
        ...
        for key, widget in self.student_entries.items():
            ...
            if isinstance(widget, JalaliDateEntry):
    ...
    def get_student_form_data(self):
        ...
        for key, widget in self.student_entries.items():
            if isinstance(widget, JalaliDateEntry):
                data[key] = widget.get_gregorian_str()
            else:
        ...
        if messagebox.askyesno("تأیید حذف", f"آیا از حذف دانش‌آموز '{student_name}' مطمئن هستید؟\n(تمام اطلاعات مالی و ارتباطی این دانش‌آموز نیز حذف خواهد شد)"):
            try:
                if self.db_manager.execute_query("DELETE FROM students WHERE id = ?", (student_id,)):
                    self.db_manager.commit(); messagebox.showinfo("موفقیت", "دانش‌آموز با موفقیت حذف شد."); self.clear_student_form(); self.load_students(); self.update_all_student_comboboxes()
                else: self.db_manager.rollback()
            except Exception as e: self.db_manager.rollback(); logging.error(f"خطا در حذف دانش‌آموز: {e}")

    # ... (بقیه توابع با تغییرات مشابه برای استفاده از JalaliDateEntry و to_jalali_str)
    ...
    def save_staff(self):
        ...
        try:
            if self.db_manager.execute_query(sql, data):
                self.db_manager.commit(); messagebox.showinfo("موفقیت", "پرسنل با موفقیت ذخیره شد."); self.clear_staff_form(); self.load_staff()
            else: self.db_manager.rollback()
    ...
    def update_staff(self):
        ...
        try:
            if self.db_manager.execute_query(sql, (*data, staff_id)):
                self.db_manager.commit(); messagebox.showinfo("موفقیت", "اطلاعات پرسنل به‌روزرسانی شد."); self.clear_staff_form(); self.load_staff()
            else: self.db_manager.rollback()
        ...
        if messagebox.askyesno("تأیید حذف", f"آیا از حذف '{staff_name}' مطمئن هستید؟"):
            try:
                if self.db_manager.execute_query("DELETE FROM staff WHERE id = ?", (staff_id,)):
                    self.db_manager.commit(); messagebox.showinfo("موفقیت", "پرسنل با موفقیت حذف شد."); self.clear_staff_form(); self.load_staff()
                else: self.db_manager.rollback()
    ...
    def create_attendance_tab(self):
        ...
        self.attendance_date_entry = JalaliDateEntry(top_frame); self.attendance_date_entry.grid(row=0, column=1, padx=5, pady=5)
        # Bind event for when date changes
        for combo in [self.attendance_date_entry.year_combo, self.attendance_date_entry.month_combo, self.attendance_date_entry.day_combo]:
            combo.bind("<<ComboboxSelected>>", self.load_attendance_list)
    ...
    def load_attendance_list(self, event=None):
        selected_date = self.attendance_date_entry.get_gregorian_str()
    ...
    def save_attendance(self):
        date = self.attendance_date_entry.get_gregorian_str()
        if not date:
        ...
        try:
            ...
            self.db_manager.commit()
            messagebox.showinfo("موفقیت", "حضور و غیاب برای تاریخ انتخاب شده با موفقیت ذخیره شد.")
        except Exception as e: self.db_manager.rollback(); messagebox.showerror("خطای دیتابیس", f"خطا در ذخیره اطلاعات حضور و غیاب: {e}")
    ...
    def create_student_finance_frame(self, parent):
        ...
        columns = ("id", "دانش‌آموز", "تاریخ پرداخت", "مبلغ", "نوع پرداخت", "توضیحات")
        headings = {"id": "ID", "دانش‌آموز": "دانش‌آموز", "تاریخ پرداخت": "تاریخ", "مبلغ": "مبلغ (ریال)", "نوع پرداخت": "نوع", "توضیحات": "توضیحات"}
        self.student_finance_tree = self._create_treeview(list_frame, columns, headings, {})
        self.student_finance_tree.grid(row=0, column=0, sticky='nsew')
        ...
        ttk.Label(form_frame, text="تاریخ پرداخت:").grid(row=1, column=0, sticky='w', padx=5, pady=5); self.finance_date_entry = JalaliDateEntry(form_frame); self.finance_date_entry.grid(row=1, column=1, sticky='ew', padx=5, pady=5)
    ...
    def load_student_finances(self):
        ...
        finance_list = [ (r[0], f"{r[1]} {r[2]}", to_jalali_str(r[3]), f"{r[4]:,.0f}", r[5], r[6]) for r in results ] if results else []
    ...
    def save_student_finance(self):
        ...
        date = self.finance_date_entry.get_gregorian_str()
        amount_str = self.finance_amount_entry.get().replace(',', '')
        ptype = self.finance_type_entry.get()
        desc = self.finance_desc_entry.get().strip()
        if not all([student_id, date, amount_str, ptype]): messagebox.showerror("خطا", "لطفا تمام فیلدها را پر کنید."); return
    ...
    def create_expense_frame(self, parent):
        ...
        ttk.Label(form_frame, text="تاریخ:").grid(row=0, column=0, sticky='w', padx=5, pady=5); self.expense_date_entry = JalaliDateEntry(form_frame); self.expense_date_entry.grid(row=0, column=1, sticky='ew', padx=5, pady=5)
    ...
    def load_expenses(self):
        ...
        results = self.db_manager.execute_query(sql, fetch="all")
        expense_list = [ (r[0], to_jalali_str(r[1]), r[2], f"{r[3]:,.0f}", r[4]) for r in results ] if results else []
        self._add_items_to_treeview(self.expense_tree, expense_list)

    def save_expense(self):
        date = self.expense_date_entry.get_gregorian_str()
        desc = self.expense_desc_entry.get().strip()
        amount_str = self.expense_amount_entry.get().replace(',', '')
    ...
    def create_communication_tab(self):
        ...
        ttk.Label(form_frame, text="تاریخ:").grid(row=1, column=0, padx=5, pady=5, sticky='w'); self.comm_date_entry = JalaliDateEntry(form_frame); self.comm_date_entry.grid(row=1, column=1, padx=5, pady=5, sticky='w')
        ...
        self.comm_tree = self._create_treeview(list_frame, ("id", "تاریخ", "موضوع", "متن پیام"), {"id": "ID", "تاریخ": "تاریخ", "موضوع": "موضوع", "متن پیام": "متن پیام"}, {"تاریخ": 100, "موضوع": 200, "متن پیام": 400})
    ...
    def load_communications(self, event=None):
        ...
        comm_list = [(r[0], to_jalali_str(r[1]), r[2], r[3]) for r in results] if results else []
        self._add_items_to_treeview(self.comm_tree, comm_list)

    def save_communication(self):
        student_id = self.student_map.get(self.comm_student_combo.get())
        date = self.comm_date_entry.get_gregorian_str()
    ...
    # ... بخش تنظیمات، پشتیبان‌گیری و کلاس لاگین بدون تغییر باقی می‌مانند ...
...
# ====================================================================================
# تابع اصلی و اجرای برنامه (بدون تغییر)
# ====================================================================================
def main():
